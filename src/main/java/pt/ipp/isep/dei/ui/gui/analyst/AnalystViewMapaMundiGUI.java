...
import pt.ipp.isep.dei.controller.analyst.AnalystViewMapaMundiController;
...
/**
 * JavaFX controller for the "mapa mundi" view used by analysts.
 *
 ...
 * and zooming centred on the mouse pointer. The actual data (station list
 * and the search coordinates) are supplied by the associated
 * {@link AnalystViewMapaMundiController} instance via
 * {@link #loadControllerInfo(List, double, double)}.</p>
 *
 * <p>The class is responsible solely for presentation and user interaction
 * (rendering, transforms and event handlers); business logic and data
 * management remain within the controller class.</p>
 */
public class AnalystViewMapaMundiGUI implements Initializable {
    ...
    private final AnalystViewMapaMundiController controller = new AnalystViewMapaMundiController();
     ...
     */
    public void loadControllerInfo(List<StationEsinf> stations, double latitude, double longitude) {
    ...
    /**
     * Handle the "back" button action by returning to the proximity search
     * view.
     *
     * @param event the action event that triggered navigation
     */
    public void backButtonOnAction(ActionEvent event) {
        uiUtils.loadFXMLScene(event, "/fxml/analyst/analystProximitySearch.fxml", "MABEC - Analyst - Proximity Search");
    }
