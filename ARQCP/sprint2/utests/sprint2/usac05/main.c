...
void run_test(int * array, int len, int nelem, int rd, int wr, int val, 
			  int * exp_arr, int exp_nelem, int exp_rd, int exp_wr, int exp_res)
{
	...
	vec[5]=nelem; 
     ...
     res=callfunc(enqueue_value,vec+7,len,vec+5,vec+1,vec+3, val);
...
void test_One()
{ 
    run_test((int[]){0,0,0}, 3, 0, 0, 0, 5,
										(int[]){5,0,0}, 1, 0, 1, 0); 
}
void test_Zero()
{ 
    run_test((int[]){0,0,0}, 3, 0, 0, 2, 5, 
									    (int[]){0,0,5}, 1, 0, 0, 0); 
}
void test_Three()                                             
{ 
    run_test((int[]){0,0,0,0}, 4, 0, 3, 3, 5,
										(int[]){0,0,0,5}, 1, 3, 0, 0); 
}
void test_Five()
{ 
    run_test((int[]){0,0,0,0}, 4, 0, 0, 3, 5,
										(int[]){0,0,0,5}, 1, 0, 0, 0); 
}

void test_Six()
{ 
    run_test((int[]){1,2,3,0}, 4, 3, 0, 3, 6,
										(int[]){1,2,3,6}, 4, 0, 0, 1); 
}

void test_Seven()
{ 
    run_test((int[]){1,2,3,6}, 4, 4, 0, 0, 7,
										(int[]){7,2,3,6}, 4, 1, 1, 1); 
} 
int main()
  { 
